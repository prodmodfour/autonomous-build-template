...
BUILD_TICKETS
    ...
    cat > BUILD_NOTES.md <<'BUILD_NOTES'
# BUILD_NOTES.md
...
BUILD_TICKETS
  ...
  printf '\nRecovery: split oversized ticket after token/context failure.\n' >> BUILD_NOTES.md
  git add BUILD_TICKETS.md BUILD_NOTES.md
...
if [[ ! -f "$AUTONOMOUS_BUILD_LOOP_STATE_DIR/context-failed" ]]; then
  ...
  printf '\nPartial oversized attempt that should be reset.\n' >> BUILD_NOTES.md
...
RUN_AGENT
  ...
  if grep -q 'Partial oversized attempt that should be reset' BUILD_NOTES.md; then
    fail "dirty context-failure changes were not reset before recovery"
  fi
)
...
if [[ ! -f "$AUTONOMOUS_BUILD_LOOP_STATE_DIR/transient-failed" ]]; then
  ...
  printf '\nPartial transient attempt that should be reset.\n' >> BUILD_NOTES.md
...
RUN_AGENT
  ...
  if grep -q 'Partial transient attempt that should be reset' BUILD_NOTES.md; then
    fail "dirty transient-failure changes were not reset before retry"
  fi
)
